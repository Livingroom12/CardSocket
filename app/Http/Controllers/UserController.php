    ...
    public function storeUser(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            broadcast(new NotificationMessage('alert', $errorMessage))->toOthers();
                ...
                if (empty($request->confirm_password)) {
                    $blankFields[] = 'confirm_password';
                }
            
                if ($validator->errors()->has('password') || $validator->errors()->has('confirm_password') || !empty($blankFields)) {
                    $errorMessage = 'No se pudo completar el registro. Verifica la información ingresada e intenta nuevamente.';
                    broadcast(new NotificationMessage('alert', $errorMessage))->toOthers();
        ...
        if ($request->has('invite')) {
            try {
                ...
                $errorMessage = "No se pudo enviar la invitación al usuario {$user->name}. Verifica la información e intenta nuevamente.";
                broadcast(new NotificationMessage('alert', $errorMessage))->toOthers();
        ...
        try{
            ...
            broadcast(new NotificationMessage('notification', $successMessage))->toOthers();
                ...
                Notification::create([
                    'user_id' => auth()->user()->id, // Assuming the notification is for the currently authenticated user
                    'notification_type' => 'notification',
                    'notification' => $successMessage,
                ]);
